...
- **ChaosEngine**: 通过这个对象来关联一个Kubernetes的应用或是Kubernetes节点。石蕊的Chaos-Operator会watch这个对象并触发混沌事件。
- **ChaosExperiment**: 这个对象包含了一组混沌事件的配置。当ChaosEngine触发混沌事件的时候，operator就会创建ChaosExperiment对象。
