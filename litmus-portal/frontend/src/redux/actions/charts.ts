...
export const loadAllCharts = (version: string) => (dispatch: Function) => {
...
export const filterCharts = (chaos: string, contributor: string) => (
  ...
  if (chaos === 'All') {
    ...
    payload = chartData.allExperimentGroups.filter((expg: ExperimentGroup) =>
      expg.experiments.some((exp: Experiment) => exp.category === chaos)
    );
  }
  if (contributor !== 'All') {
  ...
  const payload: ExperimentGroup[] = searchToken
    ? chartData.allExperimentGroups.filter(
        (expg: ExperimentGroup) =>
          tokens.every((s: string) => expg.name.toLowerCase().includes(s)) ||
          expg.experiments.some((exp: Experiment) =>
            tokens.every((s: string) => exp.name.toLowerCase().includes(s))
          )
      )
