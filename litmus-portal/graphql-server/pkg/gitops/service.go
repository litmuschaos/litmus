...
func NewService(gitOpsOperator *dbSchemaGitOps.Operator, chaosWorkflowService chaosWorkflowOps.Service, authService authentication.Service) Service {
...
func (g *gitOpsService) GitOpsNotificationHandler(ctx context.Context, cluster *dbSchemaCluster.Cluster, workflowID string) (string, error) {
	...
	if err != nil {
		log.Error("failed to updated workflow name :", err)
...
func (g *gitOpsService) EnableGitOpsHandler(ctx context.Context, config model.GitConfig) (bool, error) {
	...
	_, err := g.authService.GetProjectById(config.ProjectID)
	if err != nil {
...
func (g *gitOpsService) syncDBToGit(ctx context.Context, config GitConfig) error {
	...
	for file := range files {
		...
		log.Info("workflowID in changed File :", wfID)
		if wfID == "" {
