...
func ProcessWorkflowCreation(input *model.ChaosWorkFlowRequest, uid string, wfType *dbSchemaWorkflow.ChaosWorkflowType, r *store.StateData) error {
	...
	if r != nil {
		SendWorkflowToSubscriber(input, &uid, nil, "create", r)
	}
...
func ProcessWorkflowUpdate(workflow *model.ChaosWorkFlowRequest, uid string, wfType *dbSchemaWorkflow.ChaosWorkflowType, r *store.StateData) error {
	var Weightages []*dbSchemaWorkflow.WeightagesInput
	...
	if r != nil {
		SendWorkflowToSubscriber(workflow, &uid, nil, "update", r)
	}
	return nil
}

// ProcessWorkflowDelete deletes the workflow entry and sends delete resource request to required agent
func ProcessWorkflowDelete(query bson.D, workflow workflowDBOps.ChaosWorkFlowRequest, uid string, r *store.StateData) error {

	update := bson.D{{"$set", bson.D{{"isRemoved", true}, {"last_updated_by", uid}}}}
	err := dbOperationsWorkflow.UpdateChaosWorkflow(query, update)
	if err != nil {
		return err
	}
...
func ProcessWorkflowRunDelete(query bson.D, workflowRunID *string, workflow workflowDBOps.ChaosWorkFlowRequest, uid string, r *store.StateData) error {
	...
	if r != nil {
		SendWorkflowToSubscriber(&model.ChaosWorkFlowRequest{
			ProjectID: workflow.ProjectID,
			ClusterID: workflow.ClusterID,
		}, &uid, workflowRunID, "workflow_delete", r)
	}
	return nil
}
...
func SendWorkflowToSubscriber(workflow *model.ChaosWorkFlowRequest, uid *string, externalData *string, reqType string, r *store.StateData) {
