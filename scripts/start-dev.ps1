...
function Setup-MongoDB {
    ...
    docker rm -f m1 m2 m3 2>$null
    docker network rm mongo-cluster 2>$null
...
'@
    ...
    docker exec m1 mongo --port 27015 --eval $rsConfig
    ...
    while ($count -lt $maxWait) {
        ...
        Write-Host "Waiting for primary... ($count/$maxWait)"
...
"@
    ...
    docker exec m1 mongo --port 27015 --eval $createUserCmd
...
function Main {
    ...
    Write-Host ""
    Write-Host "To stop MongoDB, run: docker rm -f m1 m2 m3"
}
