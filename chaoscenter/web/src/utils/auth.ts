...
export function getUsername(): string {
  if (getUserDetails().token) return getUserDetailsFromJwt().username;
  return '';
}
// Returns userId from jwt token
export function getUserId(): string {
  if (getUserDetails().token) return getUserDetailsFromJwt().uid;
  return '';
}
// Returns the role of the user from jwt token
export function getUserRole(): string {
  if (getUserDetails().token) return getUserDetailsFromJwt().role;
  return '';
}
// Returns the email of the user from jwt token
export function getUserEmail(): string {
  if (getUserDetails().token) return getUserDetailsFromJwt().email ?? '';
  return '';
}
// Returns the name of the user from jwt token
export function getUserFullName(): string {
  if (getUserDetails().token) return getUserDetailsFromJwt().name ?? '';
  return '';
}
