...
// newTestRegistry creates a fresh Prometheus registry with all metrics registered.
// This avoids conflicts between tests caused by the global registry.
func newTestRegistry() *prometheus.Registry {
	reg := prometheus.NewRegistry()

	apiRequestsTotal := prometheus.NewCounterVec(
		prometheus.CounterOpts{Name: "litmus_api_requests_total", Help: "Total number of API requests"},
		[]string{"endpoint", "status", "operation_name", "operation_type"},
	)
	apiResponseTime := prometheus.NewHistogramVec(
		prometheus.HistogramOpts{Name: "litmus_api_response_time_milliseconds", Help: "API response time in milliseconds", Buckets: []float64{10, 50, 100, 200, 500, 1000, 2000, 5000}},
		[]string{"endpoint", "operation_name", "operation_type"},
	)
	apiErrorRequestsTotal := prometheus.NewCounterVec(
		prometheus.CounterOpts{Name: "litmus_api_error_requests_total", Help: "Total number of API errors"},
		[]string{"endpoint", "error_type"},
	)
	authFailuresTotal := prometheus.NewCounterVec(
		prometheus.CounterOpts{Name: "litmus_api_authentication_failures_total", Help: "Total number of authentication failures"},
		[]string{"auth_method"},
	)
	experimentsTotal := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{Name: "litmus_experiments_total", Help: "Total number of experiments"},
		[]string{"project_id", "status", "infra_id"},
	)
	experimentRunsTotal := prometheus.NewCounterVec(
		prometheus.CounterOpts{Name: "litmus_experiment_runs_total", Help: "Total number of experiment runs"},
		[]string{"project_id", "experiment_id", "experiment_name", "infra_id"},
	)
	experimentRunDuration := prometheus.NewHistogramVec(
		prometheus.HistogramOpts{Name: "litmus_experiment_run_duration_seconds", Help: "Experiment run duration in seconds", Buckets: []float64{1, 5, 10, 30, 60, 120, 300, 600, 1200, 1800}},
		[]string{"project_id", "experiment_id", "experiment_name", "infra_id"},
	)
	experimentStatus := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{Name: "litmus_experiment_status", Help: "Status of experiment runs: 1 = started, 0 = completed"},
		[]string{"project_id", "experiment_id", "experiment_name", "status", "infra_id"},
	)
	connectedAgents := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{Name: "litmus_connected_agents", Help: "Number of connected chaos agents"},
		[]string{"project_id"},
	)
	disconnectedAgents := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{Name: "litmus_disconnected_agents", Help: "Number of disconnected agents"},
		[]string{"project_id"},
	)
	totalAgents := prometheus.NewGaugeVec(
		prometheus.GaugeOpts{Name: "litmus_total_agents", Help: "Total number of registered agents"},
		[]string{"project_id"},
	)
